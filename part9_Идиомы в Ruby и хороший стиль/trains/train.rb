...
# Общий класс поезда, должен быть расширен напрямую не используется
class Train
  ...
  def validate_van!(van, recur: false)
    ...
    raise InvalidVan, 'Вагон должен быть валидным' unless recur || van.valid?(recur: true)
  end
  ...
  def validate!(recur: false)
    ...
    vans.each { |van| validate_van!(van, recur: recur) }
    # TODO
    ...
    raise InvalidRoute, 'Если указан, маршрут должен быть валидным' unless route.nil? || recur || route.valid?(recur: true)

    return if station.nil? || recur || station.valid?(recur: true)
  ...
  def change_route(new_route)
    ...
    move_station(new_route.stations.first) if new_route
  end
